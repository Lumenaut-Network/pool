...
def create_pool_account(pool_keypair):
	...
	sequence = horizon.account(funding_account_kp.address().decode()).get('sequence')
	...
	if "_links" in response:
		print("Creation of account transaction link: " + response["_links"]["transaction"]["href"])
		return True
	else:
...
def set_account_signers(pool_keypair, threshold):
	...
	if "_links" in response:
		print("Set options transaction href: ", response["_links"]["transaction"]["href"])
...
def main():
	...
	print("Pool keypair: " + pool_kp.address().decode() + " | " + pool_kp.seed().decode())
