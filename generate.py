...
# Voters is a dictionary that maps IDs to how many votes they have (before and after the donations)
voters = {}
# Donors is a dictionary that maps IDs to a list of donation_data (number%address string encoded in base64)
donors = {}
...
def donate_votes(donor_id, donation_data):
	...
	# Get the donation percentage and destination address from the base64 data string
	try:
		pct, donee_address = donation_data.split("%")
		pct = XLM_Decimal(pct)
		if pct < 0 or pct > 100: # Accept only [0,100]
			return
	except ValueError: # Split didn't produce two elements (no '%' char in the donation_string)
		return
	except InvalidOperation: # XLM_Decimal() can't produce a valid value (malformed string)
		return
...
def accounts_payouts(conn, pool_addr, inflation, size=100):
	...
	for aid, votes in voters.items():
		...
		if aid == pool_addr:
			continue
		# Add payout to the batch (minus the BASE_FEE), and start a new one if 'size' is reached
...
def main(inflation):
	...
	print(
		...
		Number of people that donated votes: " + str(len(donors)) + "\n"
	)
